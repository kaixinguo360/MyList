...
class Node(
    ...
    override fun toString(): String {
        return "Node[$id,$user,$type,$ctime,$mtime,$title,$excerpt,$linkForward,$linkBack,$linkDelete,$linkVirtual,$permissions,$nsfw,$like,$hide,$sourceUrl,$comment]"
    }
    companion object { 
        fun fromSingleNode(s: MainData?): Node? {
            if (s == null) return null
            if (s is Node) return s
            return Node(s.id, s.user, s.type, s.ctime, s.mtime, s.title, s.excerpt, s.linkForward, s.linkBack, s.linkDelete, s.linkVirtual, s.permissions, s.nsfw, s.like, s.hide, s.sourceUrl, s.comment)
        }
    }
}
